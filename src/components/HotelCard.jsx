...
const HotelCard = ({room, index}) => {
  // Add safety checks
  if (!room) {
    return <div>Room data is missing</div>;
  }

  // Fixed: Changed from room.img to room.images (plural)
    ...
    <div>
     ...
     <img 
       ...
       className='relative max-w-70 w-full rounded-xl overflow-hidden bg-white
        text-gray-500/90 shadow-[0px_4px_rgba(0,0,0,0,0.05)]' 
     />
    ...
    {index % 2 === 0 && <p className='px-3 py-1 absolute top-3 left-3 text-xs 
     ...
     <div className='p-4 pt-5'>
        ...
        <div className='flex items-center justify-between mt-4'>
            <p><span className='text-xl text-gray-800'>${room.pricePerNight || 0}</span>/night</p>
